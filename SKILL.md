...
description: Distill a complete novel into a source-isolated writing-style contract, initialize an original novel from a user theme, and write, continue, or revise that project's chapters while applying the locked style contract and persistent story state. Use for 蒸馏小说文风、用蒸馏风格开新书、写下一章或续写当前原创项目. Do not use for simple summaries, nonfiction, author role-play, source-novel continuation, passage imitation, or reproduction of source characters, worldbuilding, phrasing, or plot.
---
...
3. **文件是事实源**：不要依赖聊天记忆维护长篇连续性。已验收章节、Story Bible、当前状态、时间线和剧情线程共同构成项目事实。
...
读取 [references/08-style-pack-and-project-setup.md](references/08-style-pack-and-project-setup.md)，然后：

1. 确认用户指定的 runtime pack；未指定时列出当前可用包并让用户选择。不要自动混合多个来源包。
...
5. 建立 Story Bible、总纲、章节蓝图目录、当前状态、剧情线程、时间线和章节记录目录。
6. 从模板生成 `.agents/skills/<project-slug>-writer/SKILL.md`。其描述必须明确项目名和写章触发词，使后续自然语言请求能路由到本项目。
7. 仅在标记边界内创建或合并项目 `AGENTS.md` 指令；保留文件中所有无关内容。
8. 向用户展示建书摘要、关键假设、风格包版本和建议的第一章目标。除非用户要求，否则建书阶段不直接写完整第一章。
...
读取 [references/09-chapter-writing-and-state.md](references/09-chapter-writing-and-state.md)。优先执行项目专属 writer Skill；它是本书的运行时入口。

### C1. 定位项目

从当前目录向上查找 `NOVEL_PROJECT.md`，或使用用户提供的项目路径。确认项目 ID、当前状态、锁定风格包和目标章节。多个项目匹配且无法可靠判断时再询问用户。
...
- `PREPARE`：先读风格契约，再读章节目标、当前状态、相关 Bible 条目、未结线程、上一章摘要和必要尾段。
...
- `ACCEPT / COMMIT`：用户接受后保存正文和章节记录，原子化回写状态、时间线与剧情线程。
